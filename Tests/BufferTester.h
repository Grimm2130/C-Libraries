#if !defined(__BUFFER_TESTER_H__)
#define __BUFFER_TESTER_H__
...
static const int gBufferCapacity = PERSONS_COUNT;
...
static bool TestCreate()
{
    ...
    gBufferPtr = BufferCreate( PERSON_SIZE, gBufferCapacity );
    ...
    {        
        if( gBufferPtr->mSize == 0 )
        {
            if( gBufferPtr->mData )
            {
                if( gBufferPtr->mItemSize == PERSON_SIZE)
                {
                ...
                {
                    FAIL_MSG("Buffer Created for invalid object type. Expected type of size %d, Actual %d", PERSON_SIZE, gBufferPtr->mItemSize );
                }
            }
            else
            {
...
static bool TestPrepend()
{
    ...
    for( int i = 0; i < 100; i++ )
    {
        ...
        person = (Person_t*) BufferGet( gBufferPtr, index, PERSON_SIZE );
...
static bool TestGetter()
{
    ...
    {
        index = rand() % PERSONS_COUNT;
        Person_t* person = (Person_t*) BufferGet( gBufferPtr, index, PERSON_SIZE );
...
static bool TestSetter()
{
    ...
    for( int i = 0; i < PERSONS_COUNT; i++ )
    {
        BufferSet( gBufferPtr, i, (void *)&gPersons[i], PERSON_SIZE );
    }
